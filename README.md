...
- 可选项: 使用脚本产生自定义的文件(适合source.zip或者有bonuslab):
  ...
  + 使用命令行,进入`./script`下,`javac ./file_producer.java;java file_producer`,出现`produce files finish`提示,即为创建成功.
    默认产生lab01-lab15十五个lab,</br>
    每个lab下A-J10个问题,</br>
    每个问题一个source文件,一个test文件,一组testcase</br>
  ...
  + 如果有bonus等情况,直接到`script\file_producer.java`中修改`labs`,`problem_order`,便可修改产生的lab数,lab名与每道题对应的名字.
    + PS:  就像下面的修改,增加了两个Bonus_Lab,同时给每个Lab中的C拆分成了C1 && C2
      ...
      ``` java
          private static final List<String> labs = List.of("01", "02", "03", "04", "05", "06", "07", "08", "09", "10", "11", "12", "13", "14", "15","bonus1","bonus2");
          private static final List<String> problem_order = List.of("A", "B", "C1", "C2","D", "E", "F", "G", "H", "I", "J");
      ```

+ 执行代码和测试:
  + 使用IDEA打开文件夹,配置好Java环境与Gradle的基础上,会自动识别`build.gradle`.之后可选择test或源文件进行执行.
  + `lab_*/source/lab_*_*.java`为将要提交的源文件.
  + `lab_*/test/lab_*_*_Test.java`为对源文件进行测试的测试文件，目的为方便测试,同时便于分享测试用例.

### 实际场景:

1. A+B: lab_00_A ,测试样例

+ 这个问题较为简单,见[A+B](./lab00/src/lab_00_A.java).解决起来不复杂.
...
参见[test_for_lab00_A](./lab00/test/lab_00_A_Test.java).
+ 这样一来,我们只需要每次修改完主文件之后,run `lab_00_A_Test`, 对其进行调用,就能验证其在所有的测试用例上的正确性.</br>
测试的结果也会出现在输出中.

2. 文件输入输出重定向 part1:

+ 常见于tree,graph类的问题,debug需要的数据集都比较大,不方便直接写在代码中.
+ 比如[判断二分图](./lab00/src/lab_00_C.java),一张图可以有几十上百个node,写在内部占用空间太大.
...
1. 提示找不到Jar包.
  请安装gradle作为依赖管理的工具.然后在IDEA中`文件`-`打开`-选择`CS203_DSAA_template_java\build.gradle`-确定-作为项目打开.
2. gradle下载包很慢
  需要给gradle 换源,或者开代理.(`build.gradle`中已有换源操作)
3. 为什么要生成那么多Reader_\*_\*?

+ 为了保证每个类改掉类名就可以提交到oj,必须把Reader塞到每个文件里.
+ 而在同一个package下不能定义同名的类,只能生成这些不同名的类来辅助快读.
